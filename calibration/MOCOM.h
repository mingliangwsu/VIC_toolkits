...
/*  int          *soln_num;  TODO figure out what this still does or did originally ??  */
...
/** Function Prototypes **/
void
mocom_die(const char *, ...);

void
random_start_optimization(long *);
void
restart_optimization(const char *);

void
populate_simplex(ITEM * test_set, long *);

void
amoeba(AMOEBA_CONTEXT *);
/*   float **, float **, int *, int, int *, const char *, PARAM_RANGE *, const char *, FILE *);
*/
void
solve_model(   const float *, float *, int *);

void *
dispatch_model(const float *, float *, int *);

int check_model(DISPATCH_MODEL_STATE * state);

void
retrieve_model(DISPATCH_MODEL_STATE *);
...
PARAM_RANGE *
set_param_limits(const char *, int);

void
calc_rank_probs(void);

void
code_error(char *);

int
less_than(float *, float *, int);

int
less_than_or_equal(float *, float *, int);
